...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const getUserChatId = await supabase.from('chats').select('id').eq('userId', userId).single()

    if (!getUserChatId) {
      return new NextResponse("No chat found", { status: 404 });
    }

    const { error } = await supabase.from('messages').delete().eq('chatId', getUserChatId.data?.id)
